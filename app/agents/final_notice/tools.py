...
def build_final_notice_tools(lender_id: str) -> list[StructuredTool]:
    ...
    def get_borrower_case(borrower_id: str) -> dict:
        """Fetch the current borrower case for a borrower id."""
        borrower_case = borrower_case_service.get_borrower_case(borrower_id)
        if borrower_case is None:
            return {"found": False, "borrower_id": borrower_id}
        return {"found": True, "borrower_case": borrower_case.model_dump(mode="json")}
    ...
    return [
        ...
        StructuredTool.from_function(get_borrower_case),
